...
class focal_PDSCA(torch.optim.Optimizer):
    ...
    def __init__(self,
        ...
        super(focal_PDSCA, self).__init__(self.params, self.defaults)

    def __setstate__(self, state):
        super(focal_PDSCA, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group['params']):
                if p.grad is None:
                    continue
                d_p = torch.clamp(p.grad.data, -clip_value, clip_value) + 1 / gamma * (
    ...
    def update_regularizer(self, decay_factor=None):
        if decay_factor != None:
            ...
            print('Reducing learning rate to %.5f (%.5f) @ T=%s!' % (
            self.param_groups[0]['lr'], self.param_groups[0]['lr0'], self.steps))
